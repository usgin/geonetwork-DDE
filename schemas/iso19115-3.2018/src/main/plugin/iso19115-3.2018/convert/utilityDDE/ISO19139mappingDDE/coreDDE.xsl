<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:import href="../multiLingualCharacterStringsDDE.xsl"/>
  <xsl:import href="../dateTimeDDE.xsl"/>
  ...
  <xsl:template match="
      gfcold:typeName/gcoold:LocalName
      | gfcold:aliases/gcoold:LocalName
      | gfcold:memberName/gcoold:LocalName" priority="5" mode="from19139to19115-3.2018">
    <xsl:value-of select="."/>
  </xsl:template>
  ...
  <xsl:template match="gmd:language | gmd:locale" priority="5" mode="from19139to19115-3.2018">
    ...
    <xsl:element name="{concat($nameSpacePrefix, ':', $elementName)}">
      ...
      <xsl:apply-templates select="@*" mode="from19139to19115-3.2018"/>
      <xsl:if test="count(./@*[local-name()='nilReason'])=0">     
        <lan:PT_Locale>
    ...
    <mdb:metadataStandard>
      <cit:CI_Citation>
        <xsl:choose>
          ...
          <xsl:when
            test="matches(gcoold:CharacterString, '^ISO\s?191(15|39)((:|\.)2003/19139)?$', 'i')">
            <cit:title>
  ...
  <xsl:variable name="associatedResourceAsMetadataReferenceOnly" select="true()"/>
          ...
          <mri:metadataReference>
            ...
            <xsl:copy-of select="
                if ($uuidref != '') then
                  ...
                  gmd:MD_AggregateInformation/gmd:aggregateDataSetIdentifier/*/gmd:code/*/text()"
            />
          </mri:metadataReference>
        </xsl:if>
  ...
  <xsl:include href="defaultsDDE.xsl"/>
  <!--
  ...
  <xsl:template
    ...
    priority="5" mode="from19139to19115-3.2018"/>
