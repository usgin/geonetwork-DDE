...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:import href="utilityDDE/DDEutilities.xsl"/>
    ...
    <xsl:template match="root/mdb:MD_Metadata | mdb:MD_Metadata">
        <xsl:element name="dde:MD_Metadata"
            ...
            <dde:identificationInfo>
                ...
                <dde:abstract>
                    <xsl:choose>
                        <xsl:when test = "string-length(mdb:identificationInfo//mri:abstract/gco:CharacterString)&lt; 250">
                            <xsl:variable name="thecount" select="(250-string-length(mdb:identificationInfo//mri:abstract/gco:CharacterString))div 56"/>
                            <xsl:variable name="theextra">
                                <xsl:choose>
                                    <xsl:when test="$thecount &lt; 1">
                                        <xsl:value-of select="string('ERROR, resource must have an abstract at least 250 characters long.')"/>
                                    </xsl:when>
                                    <xsl:when test="$thecount &lt; 2">
                                        <xsl:value-of select="string('ERROR, resource must have an abstract at least 250 characters long.')"/>
                                        <xsl:value-of select="string('ERROR, resource must have an abstract at least 250 characters long.')"/>
                                    </xsl:when>
                                    <xsl:when test="$thecount &lt; 3">
                                        <xsl:value-of select="string('ERROR, resource must have an abstract at least 250 characters long.')"/>
                                        <xsl:value-of select="string('ERROR, resource must have an abstract at least 250 characters long.')"/>
                                        <xsl:value-of select="string('ERROR, resource must have an abstract at least 250 characters long.')"/>
                                    </xsl:when>
                            ...
                            <xsl:value-of
                                select="concat(mdb:identificationInfo//mri:abstract/gco:CharacterString,string('....'), $theextra) "/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of
               ...
               -->
                ...
                <xsl:for-each select="mdb:identificationInfo//srv:coupledResource/srv:SV_CoupledResource">
                    <dde:associatedResource>
                        <dde:resourceCitation>
                            <xsl:choose>
                                <xsl:when  test="srv:resourceReference[1]/cit:CI_Citation">
                                    <xsl:for-each select="srv:resourceReference[1]/cit:CI_Citation">
                                    <dde:title>
                                        <xsl:choose>
                                ...
                                <xsl:otherwise>
                                    <xsl:for-each select="srv:resource[1]//cit:CI_Citation">
                                        <dde:title>
                                            <xsl:choose>
                                                <xsl:when test="cit:title/gco:CharacterString">
                                                    <xsl:value-of select="cit:title/gco:CharacterString"/>
                                        ...
                                        <dde:description>
                                            ...
                                            <xsl:for-each select = ".*/ancestor::srv:operation/srv:SV_OperationMetadata">
                                                <xsl:call-template name="getText">
                                                    <xsl:with-param name="theNode" select="."/>
                                                </xsl:call-template>
                                            </xsl:for-each>
                            ...
                            <dde:relation>
                                <xsl:variable name="coupledResourceRelation">
                                    <xsl:value-of select="string('Service coupled resource. ')"/>
                                    <xsl:if test="srv:scopedName">
                                        <xsl:value-of select="srv:scopedName/gco:ScopedName"/>
                                    </xsl:if>
                                </xsl:variable>
                                <xsl:value-of select="normalize-space($coupledResourceRelation)"/>
                            </dde:relation>
                    </dde:associatedResource>
                </xsl:for-each>
                ...
                <xsl:for-each select="mdb:distributionInfo/mrd:MD_Distribution">
                    ...
                    <xsl:if test="//srv:connectPoint">
                        <!-- put service connect points in dde distribution -->
...
<!-- use local-name here to avoid problems with srv2.0 vs. srv2.1.  Geonetwork uses srv2.1, 
                    ...
                    <dde:serviceIdentificationInfo>
                        ...
                        <xsl:for-each select="*[local-name(.)='containsOperations']">
                            <dde:containOperations>
                                <xsl:variable name="operationdesc">
                                    <xsl:for-each select="*[local-name(.)='SV_OperationMetadata']/*">
                                        <xsl:if test=".//gco:CharacterString and local-name(.)!='connectPoint'">
                                        ...
                                        <xsl:for-each select=".[local-name(.) != 'connectPoint']/child::*//@codeListValue">
                                            <xsl:value-of select="local-name(./parent::*)"/>
                                            <xsl:text>:</xsl:text>
                                    ...
                                    <xsl:if test="count(local-name(.)='connectPoint')>0">
                                        <xsl:value-of select="string('  connectPoints are in Distribution section')"/> 
                                    </xsl:if>
                                </xsl:variable>
                                <xsl:variable name="dquote">"</xsl:variable>
                                <!-- This construction generates text with a bunch of line feeds and double quotes. 
                                    I can't figure out why the double quotes are getting inserted, but 
                                    this is just to get them gone-->
                                <xsl:value-of select="normalize-space(translate($operationdesc, $dquote,''))"/>
                        ...
                        <xsl:if test="count(*[local-name(.)='accessProperties']) + count(*[local-name(.)='profile']) + count(*[local-name(.)='serviceStandard']) > 0">
                            <dde:accessProperties>
                                <xsl:variable name="accessText">
                                 <xsl:for-each
                                 ...
                                 <xsl:for-each select="*[local-name(.)='profile']">
                                 ...
                                 <xsl:for-each select="*[local-name(.)='serviceStandard']">
                                    <xsl:value-of select="string('serviceStandard: ')"/>
                                    <xsl:call-template name="getText">
                                        <xsl:with-param name="theNode" select="."/>
                                    </xsl:call-template>
                                    <xsl:if test="position() > 2">
                                ...
                                <xsl:variable name="dquote">"</xsl:variable>
                                <xsl:value-of select="normalize-space(translate($accessText, $dquote,''))"/>
                                
                            </dde:accessProperties>
                        </xsl:if>
                            
                        <xsl:for-each select = "srv:operatesOn//cit:CI_Citation | srv:operatedDataset/cit:CI_Citation">
                            <xsl:choose>
                                <xsl:when test=".//mcc:MD_Identifier">
                                    <dde:operatedDataset >
                                        <xsl:apply-templates select=".//mcc:MD_Identifier[1]"/>
                                    </dde:operatedDataset>
                                </xsl:when>
                                <xsl:otherwise>
                                   <dde:operatedDataset >
                                       <dde:authority>
                                           ...
                                           <dde:role>distributor</dde:role>
                                       </dde:authority>
                                       <dde:code>
                                           <xsl:value-of select ="cit:onlineResource[1]/cit:CI_OnlineResource/cit:linkage/gco:CharacterString"/>
                                       </dde:code>
                                        <dde:description>
                                            <xsl:variable name="parentname">
                                                <xsl:choose>
                                                    <xsl:when test="local-name(./parent::*) = 'operatedDataset'">
                                                        <xsl:value-of select="string('operatedDataset/CI_Citation')"/>
                                                    </xsl:when>
                                                    <xsl:otherwise>
                                                        <xsl:value-of select="string('operatesOn/MD_DataIdentification/CI_Citation')"/>
                                                    </xsl:otherwise>
                                                </xsl:choose>
                                            </xsl:variable>
                                            <xsl:value-of select="concat(string('Extracted from '), $parentname)"/>
                                        </dde:description>
                                   </dde:operatedDataset>
                                </xsl:otherwise>
                            </xsl:choose>
                        ...
                        <xsl:for-each select = "srv:operatesOn/@*[local-name()='href'] | srv:operatedDataset/@*[local-name()='href']">
                                    <dde:operatedDataset >
                                        <dde:code>
                                            <xsl:value-of select ="."/>
                                        </dde:code>
                                        <dde:description>
                                            <xsl:variable name="parentname">
                                                <xsl:choose>
                                                    <xsl:when test="local-name(./parent::*) = 'operatedDataset'">
                                                        <xsl:value-of select="string('operatedDataset/href')"/>
                                                    </xsl:when>
                                                    <xsl:otherwise>
                                                        <xsl:value-of select="string('operatesOn/href')"/>
                                                    </xsl:otherwise>
                                                </xsl:choose>
                                            </xsl:variable>
                                            <xsl:value-of select="concat(string('Extracted from '), $parentname)"/>
    ...
    <xsl:template match="cit:CI_OnlineResource">
        <xsl:variable name="opname2" select ="local-name(./parent::*/preceding-sibling::srv:operationName) = 'operationName'"/>
        <xsl:variable name="srvtype"  select ="string(//cit:CI_OnlineResource/ancestor::*/preceding-sibling::srv:serviceType/gco:ScopedName)"/>
        <xsl:if test="$opname2 or cit:name">
            <dde:title>
                <xsl:value-of select="child::cit:name/gco:CharacterString"/>
                <xsl:if test="$opname2">
                    <xsl:value-of select = "concat(string('operation: '),./parent::*/preceding-sibling::srv:operationName/child::*)"/>
                </xsl:if>
            </dde:title>
        </xsl:if>
        <xsl:if test="cit:applicationProfile or string-length($srvtype) &gt; 0">
            <dde:applicationProfile>
                <xsl:if test="string-length($srvtype) &gt; 0">
                        <xsl:value-of select="normalize-space($srvtype)"/>
                </xsl:if>
                <xsl:if test="cit:applicationProfile/gco:CharacterString">
                    <xsl:value-of select="string(':  ')"/>
                </xsl:if>
        ...
        <xsl:if test="cit:description">
            <dde:description>
                <xsl:value-of select="normalize-space(child::cit:description)"/>
            </dde:description>
        </xsl:if>
